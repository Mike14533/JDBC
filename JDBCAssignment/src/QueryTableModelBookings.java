...
class QueryTableModelBookings extends AbstractTableModel
{
	...
	public QueryTableModelBookings(){
	...
	public void refreshFromDB( Statement stmt1)
	{
		...
		try{	
			...
			//rs = stmt.executeQuery("select  booking.id,booking.dog_name, booking.date, booking.time, customer.firstName as first_name, customer.lastName as last_name, customer.phoneNumber as phone_number, dog.dogName as dogs_name from booking INNER JOIN customer ON customer.id = booking.id INNER JOIN dog ON dog.id = booking.id;");
